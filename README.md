# ml-based-orbital-correction
course project for machine learning for physics ,iith
contains python code to predict the orbit with better precision than GMAT uisng ml.
ajisai.script file works in GMAT 2016a

plotly based orbit isnt visible in github but is available in google colab 

please download all the datsets and upload to colab and run

if the notebook isnt visible, kindly give the url to https://nbviewer.org/ to view full notebook
